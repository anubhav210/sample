...
public class S3OutputStream extends OutputStream {
    ...
    public S3OutputStream(AmazonS3 s3Client, String bucket, String path,ObjectMetadata objectMetadata) {
        this.s3Client = s3Client;
        this.bucket = bucket;
        this.path = path;
        this.buf = new byte[BUFFER_SIZE];
        ...
        if (uploadId == null) {
            final InitiateMultipartUploadRequest request = new InitiateMultipartUploadRequest(this.bucket, this.path)
                    ...
                    if(objectMetadata!=null){
                        request.withObjectMetadata(objectMetadata);
                    }
    ...
    public void close() {
        if (this.open) {
            ...
            if (this.uploadId != null) {
                ...
                objectMetadata.setContentLength(this.position);
                final PutObjectRequest request = new PutObjectRequest(this.bucket, this.path, new ByteArrayInputStream(this.buf, 0, this.position), objectMetadata)
                        .withCannedAcl(CannedAccessControlList.BucketOwnerFullControl);
